...
def calc_distances(arr, pt):
    return sum([abs(a-pt) for a in arr])


def calc_distances_b(arr, pt):
    def get_dist(a):
        dist = abs(a-pt)
        return dist * (dist+1) / 2
    return int(sum([get_dist(a) for a in arr]))


def run_part_a(crabs) -> int:
    ...
    for x in range(-900, 900):
        ...
        if target < 0:
            continue
        distance = calc_distances(crabs, target)
        if distance < min_distance:
            print(f"Got new min {target} at {distance}")
...
def run_part_b(crabs) -> int:
    ...
    for x in range(-500, 600):
        ...
        if target < 0:
            continue
        distance = calc_distances_b(crabs, target)
        if distance < min_distance:
            print(f"Got new min {target} at {distance}")
            min_distance = distance
            res = target
...
def test_part_a():
    expected = 37
    actual = run_part_a([16, 1, 2, 0, 4, 2, 7, 1, 2, 14])
